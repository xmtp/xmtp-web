...
import type { WasmGroup, WasmMessage } from "@/types";
...
export class Conversation {
  ...
  constructor(client: Client, group: WasmGroup) {
    this.#client = client;
    this.#group = group;
  }
  ...
  messages(options?: any): any[] {
    return this.#group
      .findMessages(options)
      .map(
        (message) => new DecodedMessage(this.#client, message as WasmMessage),
      );
  }
}
